# The Tools Used in `ngBoilerplate-sass`

## Introduction

`ngBoilerplate-sass` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `ngBoilerplate-sass` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`ngBoilerplate-sass` uses git to manage its codebase. While in theory you don't have
to use Git once you download `ngBoilerplate-sass`, this project makes the assumption
...
While `ngBoilerplate-sass` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `ngBoilerplate-sass` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `ngBoilerplate-sass`; Grunt is
...
to get started with `ngBoilerplate-sass`. But for those curious or looking to go a
...
Grunt is the engine behind `ngBoilerplate-sass`. It's the magic that makes it move.
...
`ngBoilerplate-sass` comes with a `bower.json` file that looks something like this:
...
in `ngBoilerplate-sass` by default in there to see what I mean.
