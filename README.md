# [ngBoilerplate-sass](http://justyn-clark.github.com/ngBoilerplate-sass) [![Build Status](https://api.travis-ci.org/ngbp/ngbp.png?branch=v0.3.2-release)](https://travis-ci.org/ngbp/ngbp)
...
```sh
$ git clone git://github.com/justyn-clark/ngBoilerplate-sass
$ cd ngBoilerplate-sass
...
Finally, open `file:///path/to/ngBoilerplate-sass/build/index.html` in your browser.

Happy hacking!

## Purpose

`ngBoilerplate-sass` is designed to make life easy by providing a basic framework
...
ngBoilerplate-sass also comes prepackaged with the most popular design frameworks
...
The principal goal of `ngBoilerplate-sass` is to set projects up for long-term
success.  So `ngBoilerplate-sass` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `ngBoilerplate-sass`.

But `ngBoilerplate-sass` is not an example of an AngularJS app: this is a
...
getting `ngBoilerplate-sass` up and running. Though `ngBoilerplate-sass` is really simple
...
`ngBoilerplate-sass` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/justyn-clark/ngBoilerplate-sass my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `ngBoilerplate-sass`, like Twitter Bootstrap
...
Technically, `ngBoilerplate-sass` is now ready to go.
...
`ngBoilerplate-sass`. What follows in this section is a quick introduction to the
...
* `delta:scss` - When any `*.scss` file within `src/` changes, the
  ...
  `build/assets/ngBoilerplate-sass.css`.
...
`ngBoilerplate-sass` also includes [Live Reload](http://livereload.com/), so you no
...
default with `ngBoilerplate-sass`) you need to tell Live Reload to allow it. Go to
...
See the [issues list](http://github.com/justyn-clark/ngBoilerplate-sass/issues). And
