...
As `ngBoilerplate-sass` is quite minimal, take a look at the two provided submodules
...
```js
angular.module( 'ngBoilerplate-sass', [
  'templates-app',
  'templates-common',
  'ngBoilerplate-sass.home',
  'ngBoilerplate-sass.about'
  'ui.router',
  'ui.route'
])
```
...
.controller( 'AppCtrl', function AppCtrl ( $scope, $location ) {
  $scope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams){
    if ( angular.isDefined( toState.data.pageTitle ) ) {
      $scope.pageTitle = toState.data.pageTitle + ' | ngBoilerplate-sass' ;
    }
  });
})
```
...
One of the design philosophies of `ngBoilerplate-sass` is that tests should exist
